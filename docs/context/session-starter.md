...
**Approach A roadmap (approved):** M8 Commission UI → M9 tag DB authoring → M10 diagnostics → **M11** distribution (signing, notarization, auto-update, Linux AppImage) — all shipped through 1.3.x. **Next:** M12 installer trust (secrets deferred) → M13 Operate polish → M14 Commission depth — see `docs/superpowers/specs/2026-09-04-m12-installer-trust-design.md` and siblings. Unit programming remains out of scope.
